...
def is_on_cooldown(state: dict, author: str) -> tuple[bool, int]:
...
def pick_cat_name(maze_number: int, used_names: list[str]) -> str:
...
def build_win_banner(state: dict) -> str:
    """
    HTML win banner using the actual cat GIF from the completed maze.
    Rendered inline in the README — same visual language as the maze table.
    """
    cat_colour   = state["cat_colour"]
    ...
    # Build a row of running cats across the banner
    cat_row = "".join(
        f'<img src="{cat_run_gif}" width="64" height="64">'
        for _ in range(9)
    )

    return f"""<table width="100%"><tbody>
<tr>
  <td align="center" style="background:#1e1e2a; padding: 8px 0;">
    {cat_row}
  </td>
</tr>
<tr>
  <td align="center" style="background:#1e1e2a; padding: 12px 24px;">
    <img src="{cat_idle_gif}" width="80" height="80">
    <br/>
...
def build_hall_of_fame(hall: list[dict]) -> str:
    if not hall:
...
def build_readme(state: dict) -> str:
    ...
    how_it_works = f"""- click a direction above → a github issue opens, pre-filled
...
- one move per person every {COOLDOWN_MINUTES} minutes — others can move in the meantime"""
...
_click a direction · opens a pre-filled issue · just hit submit_
...
def parse_direction(title: str) -> str | None:
    title = title.strip()
...
def attempt_move(state: dict, direction: str) -> tuple[bool, str]:
    grid   = state["grid"]
